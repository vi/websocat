...
macro_rules! wt {
    ...
    ($core:ident, $s1:expr, $s2:expr,opts = $opts:expr,delay = $ms:expr) => {{
        ...
        delay.and_then(|()| wt!(wss, websocat, h2))
    }};
...
fn trivial() {
    prepare!(core);
    let prog = wt!(core, "literal:qwerty", "assert:qwerty", noopts, nodelay);
    run!(core, prog);
}
...
fn unix() {
    ...
    let prog2 = wt!(core, "unix-c:zxc", "assert:qwert3y", noopts, delay = 200);
...
fn abstract_() {
    prepare!(core);
    let prog1 = wt!(core, "literal:qwert4y", "abstract-l:zxc", noopts, nodelay);
    let prog2 = wt!(
        core,
        "abstract-c:zxc",
        "assert:qwert4y",
