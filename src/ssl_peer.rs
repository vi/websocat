...
lazy_static! {
    static ref KEYLOG_WRITERS: Mutex<HashMap<PathBuf, Arc<Mutex<File>>>> =
        Mutex::new(HashMap::new());
}

fn get_keylog_writer(keylog_path: &str) -> std::io::Result<Arc<Mutex<File>>> {
    let path = PathBuf::from(keylog_path);

    let mut writers = KEYLOG_WRITERS.lock().map_err(|_| {
        std::io::Error::new(
            std::io::ErrorKind::Other,
            "global TLS keylog writer registry is poisoned",
        )
    })?;
    ...
    writers.insert(path, Arc::clone(&writer));
    Ok(writer)
}
...
pub fn configure_keylog(ctx: &mut openssl::ssl::SslConnectorBuilder, keylog_path: &str) {
    match get_keylog_writer(keylog_path) {
        Ok(writer) => {
