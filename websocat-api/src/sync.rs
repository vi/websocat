    ...
    async fn run(
        ...
        if let Some(ref mut mc) = multiconn {
            if let Some(ref mut cag) = mc.you_are_called_not_the_first_time {
                let tmp = cag
                    .downcast_mut::<Option<tokio::sync::mpsc::Receiver<super::Bipipe>>>()
            ...
            Node::run(self, ctx, allow_multiconnect, move |pipe| {
                let r = match pipe.r {
                    ...
                    Source::Datagrams(rr) => {
                        ...
                        super::Source::Datagrams(Box::pin(strgw))
                    }
                    Source::None => super::Source::None,
                };
                ...
                let w = match pipe.w {
                    ...
                    Sink::Datagrams(ww) => {
                        ...
                        super::Sink::Datagrams(Box::pin(sinkgw))
                    }
                    Sink::None => super::Sink::None,
                };
